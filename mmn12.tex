...
% Macro to create a table from CSV with dynamic handling
\newcommand{\CreateCSVTable}[3][2]{%
    ...
    \begin{table}[htbp]
        ...
        \pgfplotstabletypeset[
            ...
            string type, % Treat all columns as strings by default
            % Dynamically apply precision to numeric columns
            columns/.style={
                % Treat each column dynamically
                column type={c},
                postproc cell content/.append code={%
                    \begingroup
                    \pgfkeys{/pgf/number format/fixed,/pgf/number format/precision=#1}%
                    \ifnum\pdfstrcmp{\pgfplotstablecolname}{Classifier}=0%
                        \pgfplotsretval% Leave strings as-is
                    \else%
                        \expandafter\pgfmathprintnumber\expandafter{\pgfplotsretval}%
                    \fi%
                    \endgroup
                }
            }
...
הטבלה:
\EN
\CreateCSVTable[2]{data/results_extractor_Orb.csv}{Results for Extractor ORB}
